...
import static com.golem.golemmod.utils.Colors.getRarityCode;
...
//			Separate commands later!
		...
		if (args.length == 2) {
			...
			int tiers_needed = (int) Math.floor(Math.pow(2, end_tier - 1) - Math.pow(2, start_tier - 1));
			...
			getAttributePrice(attribute, armor_type, tiers_needed);
		}
		if (args.length == 3) {
			String armor_type = args[0];
			String attribute = args[1];
			int end_tier = Integer.parseInt(args[2]);
			System.out.println(armor_type);
			System.out.println(attribute);
			System.out.println(end_tier);
			int start_tier = 0;
			int tiers_needed = (int) Math.floor(Math.pow(2, end_tier - 1) - Math.pow(2, start_tier - 1));
			getAttributePrice(attribute, armor_type, tiers_needed);
		}
	...
	public void getAttributePrice(String attribute, String key, int tiers_needed) {
		System.out.println(AttributePrices.get("BOOTS").containsKey("VETERAN"));
		...
		addChatMessage(EnumChatFormatting.AQUA + ToolTipListener.TitleCase("Upgrading " + attribute + " to " + (int) (Math.log(tiers_needed) / Math.log(2)) + 1 + " on " + key));
